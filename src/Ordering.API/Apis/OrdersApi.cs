...
public static class OrdersApi
{
    ...
    {
        return await ShipOrCancelOrderAsync(requestId, command, services, c => c.OrderNumber, "Cancel order failed to process.");
    }
    ...
    public static async Task<Results<Ok, BadRequest<string>, ProblemHttpResult>> ShipOrderAsync(
        ...
        [AsParameters] OrderServices services)
    {
        return await ShipOrCancelOrderAsync(requestId, command, services, c => c.OrderNumber, "Ship order failed to process.");
    }
    ...
    private static async Task<Results<Ok, BadRequest<string>, ProblemHttpResult>> ShipOrCancelOrderAsync<TCommand>(
        ...
        Func<TCommand, int> orderNumberAccessor,
        string problemDetail) where TCommand : IRequest<bool>
    {
        ...
        if (!commandResult)
        {
            return TypedResults.Problem(detail: problemDetail, statusCode: 500);
        }
