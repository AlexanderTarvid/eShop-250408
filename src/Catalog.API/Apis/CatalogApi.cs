...
public static class CatalogApi
{
    ...
    public static async Task<Ok<PaginatedItems<CatalogItem>>> GetAllItems(
        ...
        [Description("The ordering of items to return (name or price)")] string orderBy)
    {
        ...
        var orderedRoot = orderBy?.ToLowerInvariant() switch
        {
            "price" => root.OrderBy(c => c.Price),
            _ => root.OrderBy(c => c.Name)
        };

        var itemsOnPage = await orderedRoot
